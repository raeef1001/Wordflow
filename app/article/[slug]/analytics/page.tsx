...
import ArticleAnalyticsDashboard from '@/components/analytics/ArticleAnalyticsDashboard';
...
export default function ArticleAnalyticsPage() {
  ...
  const fetchArticle = async () => {
    try {
      ...
      if (!isAuthor && !isAdmin) {
        setError('You do not have permission to view analytics for this article');
      } else {
        setAuthorized(true);
      }
    } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold flex items-center">
          ...
          Analytics for "{article.title}"
        </h1>
        <p className="text-gray-600 mt-2">
          Detailed performance metrics and reader engagement statistics
        </p>
      </div>

      {authorized && (
        <div className="mb-8">
          <ArticleAnalyticsDashboard articleId={article.id} />
        </div>
      )}
    </div>
  );
